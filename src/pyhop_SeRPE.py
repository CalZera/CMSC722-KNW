"""
pyhop_SeRPE, version 1.2.2 -- a simple SHOP-like planner written in Python.
...
pyhop_SeRPE should work correctly in both Python 2.7 and Python 3.2.
For examples of how to use it, see the example files that come with pyhop_SeRPE.

pyhop_SeRPE provides the following classes and functions:

- foo = State('foo') tells pyhop_SeRPE to create an empty state object named 'foo'.
  To put variables and values into it, you should do assignments such as
  foo.var1 = val1

- bar = Goal('bar') tells pyhop_SeRPE to create an empty goal object named 'bar'.
  To put variables and values into it, you should do assignments such as
  bar.var1 = val1
...
- declare_operators(o1, o2, ..., ok) tells pyhop_SeRPE that o1, o2, ..., ok
...
- declare_methods('foo', m1, m2, ..., mk) tells pyhop_SeRPE that m1, m2, ..., mk
...
- pyhop_SeRPE(state1,tasklist) tells pyhop_SeRPE to find a plan for accomplishing tasklist
  (a list of tasks), starting from an initial state state1, using whatever
  methods and operators you declared previously.

- In the above call to pyhop_SeRPE, you can add an optional 3rd argument called
  'verbose' that tells pyhop_SeRPE how much debugging printout it should provide:
- if verbose = 0 (the default), pyhop_SeRPE returns the solution but prints nothing;
...
# pyhop_SeRPE's planning algorithm is very similar to the one in SHOP and JSHOP
# (see http://www.cs.umd.edu/projects/shop). Like SHOP and JSHOP, pyhop_SeRPE uses
# HTN methods to decompose tasks into smaller and smaller subtasks, until it
# finds tasks that correspond directly to actions. But pyhop_SeRPE differs from 
# SHOP and JSHOP in several ways that should make it easier to use pyhop_SeRPE
# as part of other programs:
# 
# (1) In pyhop_SeRPE, one writes methods and operators as ordinary Python functions
...
#     pyhop_SeRPE uses state-variable representation: a state is a Python object
...
#     g = Goal()
#     g.loc['b'] = 'r2'
#     Like most HTN planners, pyhop_SeRPE will ignore g unless you explicitly
...
#     In the same fashion, you could tell pyhop_SeRPE to achieve any one of
#     several different goals, or to achieve them in some desired sequence.
# 
# (4) Unlike SHOP and JSHOP, pyhop_SeRPE doesn't include a Horn-clause inference
...
# pyhop_SeRPE. To run them, launch python and type "import blocks_world_examples"
...
############################################################
# Commands to tell pyhop_SeRPE what the operators and methods are
...
def declare_operators(*op_list):
    """
    Call this after defining the operators, to tell pyhop_SeRPE what they are. 
...
def declare_methods(task_name, *method_list):
    """
    Call this once for each task, to tell pyhop_SeRPE what the methods are.
...
def pyhop_SeRPE(state, tasks, verbose=0):
    """
    Try to find a plan that accomplishes tasks in state. 
    If successful, return the plan. Otherwise return False.
    """
    if verbose > 0: print('** pyhop_SeRPE, verbose={}: **\n   state = {}\n   tasks = {}'.format(verbose, state.__name__, tasks))
